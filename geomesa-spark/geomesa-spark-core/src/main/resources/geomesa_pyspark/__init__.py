...
def process_executor_packages(executor_packages, tmp_path=None):
    ...
    driver_packages = { module for _, module, package in pkgutil.iter_modules() if package == True }
    executor_files = []
...
def zip_dir(path, zip_path):
    path_len = len(os.path.dirname(path)) + 1
    with zipfile.PyZipFile(zip_path, 'w') as writer:
        for root, _, files in os.walk(path):
            for file in files:
                full_path = os.path.join(root, file)
                archive_path = full_path[path_len:]
